...
function normalizeKeywords(article, topic) {
  const raw=Array.isArray(article.keywords)?article.keywords.map(String).map(x=>x.trim()).filter(Boolean):[];
  const title=String(article.title||topic).trim();
  const derived=[title, topic, `how to ${topic.toLowerCase().replace(/^how to\s+/i,'')}`, 'home cleaning tips', 'cleaning guide'];
  return [...new Set([...raw,...derived].map(x=>x.trim()).filter(Boolean))].slice(0,8);
}
