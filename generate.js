...
const usTopics = ['How to Clean Soap Scum Off Glass Shower Doors','DIY Natural Floor Cleaner Recipes for Common US Floor Types','How to Remove Hard Water Stains From Shower Glass and Tile','How to Clean a Smelly Kitchen Sink and Garbage Disposal','How to Clean a Stainless Steel Sink Without Scratches','How to Get Pet Hair Out of Carpet and Upholstery','How to Deep Clean a Bathroom: A Step-by-Step Checklist','How to Clean Greasy Kitchen Cabinets Without Damaging the Finish','How to Remove Sticky Residue From Hardwood and Vinyl Floors','How to Clean Dust From Air Vents and HVAC Registers'];
...
async function requestGemini(prompt) {
  ...
  for (let modelIndex=0; modelIndex<fallbackModels.length; modelIndex++) {
    ...
    for (let attempt=1; attempt<=2; attempt++) {
      const response=await fetch('https://generativelanguage.googleapis.com/v1beta/openai/chat/completions',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${apiKey}`,'x-goog-api-client':'mrglowra-blog/2.2'},body:JSON.stringify({model:currentModel,temperature:.72,messages:[{role:'system',content:'You are a careful US SEO editor for a neutral home-cleaning publication. Accuracy, usefulness, safety and natural writing matter more than hype.'},{role:'user',content:prompt}]})});
      if(response.ok){
        const data=await response.json();
        const raw=data.choices?.[0]?.message?.content?.trim();
        if(!raw)throw new Error(`Gemini returned no article content using ${currentModel}`);
        console.log(`Gemini generation succeeded with ${currentModel} (attempt ${attempt}).`);
        return raw;
      }
      const body=await response.text();
      lastError=`${response.status} ${body}`;
      console.warn(`Gemini ${currentModel} attempt ${attempt} failed: ${lastError}`);
      if(!transientStatus(response.status))throw new Error(`Gemini API error: ${lastError}`);
      if(attempt<2)await sleep(5000*attempt);
    }
    if(modelIndex<fallbackModels.length-1){
      console.warn(`Falling back from ${currentModel} to ${fallbackModels[modelIndex+1]}.`);
      await sleep(3000);
    }
  }
